...
def set_daily_slot_limit(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Validate slot limit range (0 to 1000)
            if new_limit < 0 or new_limit > 1000:
                error_message = "Slot limit must be between 0 and 1000."
            else:
                # Save to settings (or a model in a real app)
                settings.OVERALL_SLOT_LIMIT = new_limit
                settings.SLOT_LIMIT_END_DATE = end_date
                messages.success(request, 'Slot limit successfully updated.')
                return redirect('admin_dashboard')
                
        except (ValueError, TypeError):
            error_message = "Please enter a valid number for slot limit."
